 ...
 */var LandsatMasker=exports.LandsatMasker=function(_Masker){_inherits(LandsatMasker,_Masker);function LandsatMasker(){_classCallCheck(this,LandsatMasker);return _possibleConstructorReturn(this,Object.getPrototypeOf(LandsatMasker).apply(this,arguments))}_createClass(LandsatMasker,[{key:'getCloudMask',/**
   ...
   * @return  {ndarray}   mask                  a ndarray mask
   */},{key:'getMultiMask',value:function getMultiMask(conditions){var _this2=this;if(!Array.isArray(conditions)||conditions.length<1){throw new Error('Mask conditions is invalid.')}var masks=conditions.forEach(function(condition){switch(condition.type){case'cloud':return _this2.getCloudMask(condition.confidence);case'cirrus':return _this2.getCirrusMask(condition.confidence);case'veg':return _this2.getVegMask(condition.confidence);case'snow':return _this2.getSnowMask(condition.confidence);default:throw new Error('Condition type '+condition.type+' unrecongized');}});var x=this.rasterSize.x;var y=this.rasterSize.y;var initialMask=ndarray(new Uint32Array(x*y*4),[x,y]);var selection=initialMask.hi(x,y).lo(0,0);for(var i=0;i<selection.shape[0];++i){for(var j=0;j<selection.shape[1];++j){selection.set(i,j,1)}}return masks.reduce(function(multiMask,mask){ops.band(multiMask,multiMask,mask);return multiMask},initialMask)}}]);return LandsatMasker}(Masker);